...
// CRITICAL: Import all necessary Supabase functions
import { 
    saveTreeToSupabase, 
    getAllTreeIds, // Needed for fetching list
    deleteTree,    // Needed for deleting a tree
    onAuthChange,
} from '../Services/supabase'; // Corrected import path

const DEFAULT_TREE_ID = "My_First_Tree";

// Placeholder for custom UI interaction (must replace window.confirm/alert)
const showCustomConfirmation = (message, onConfirm) => {
    console.log("Confirmation requested (must be a custom modal UI):", message);
    // Note: We use the native confirm only as a fallback for this environment. 
    // In a production app, replace this with a custom React modal.
    if (confirm(message)) { 
        onConfirm();
    }
};
...
const showCustomAlert = (message) => {
    console.error("Alert requested (must be a custom modal UI):", message);
    // Note: We use the native alert only as a fallback for this environment. 
    // In a production app, replace this with a custom React component.
    alert(message); 
};
...
const Home = () => {
    ...
    // Initialize user as null, will be set by the onAuthChange listener
    const [user, setUser] = useState(null); 
    const [treeIds, setTreeIds] = useState([]);
    const [newSurname, setNewSurname] = useState('');
    const [loading, setLoading] = useState(true);

    // --- Authentication Listener (Supabase) ---
    const fetchTreeIds = useCallback(async (currentUser) => {
        if (!currentUser) return;
        setLoading(true);
        try {
            // Use the imported Supabase service function
            ...
            showCustomAlert(`Failed to load tree list: ${error.message}`);
        } finally {
            setLoading(false);
        }
    }, []);
    ...
    useEffect(() => {
        // Use the onAuthChange from the service file
        const unsubscribe = onAuthChange((currentUser) => {
            setUser(currentUser);
            if (currentUser) {
                // User is logged in, now fetch trees
                // Note: fetchTreeIds is a dependency because it's defined outside and called here.
    ...
    const createDefaultTree = async (treeId) => {
        ...
        // Save the minimal tree structure using the Supabase service function
        await saveTreeToSupabase(treeId, { nodes: cleanedNodes, edges: [] });
    }
    ...
    const handleCreateTree = useCallback(async () => {
        ...
        if (treeIds.includes(treeId)) {
            showCustomAlert(`A family tree named '${treeId}' already exists.`);
            return;
        }
        ...
        // The error message from Supabase is now shown to the user
    ...
    const handleDeleteTree = (treeId) => {
        showCustomConfirmation(
            `Are you sure you want to delete the '${treeId}' family tree? This cannot be undone.`,
            async () => {
                try {
                    // deleteTree is imported from supabase.js and is correct
                    await deleteTree(treeId);
                    // Reload the list of trees
                    await fetchTreeIds(user); 
                } catch (error) {
                    showCustomAlert(`Error deleting tree: ${error.message}`);
                }
            }
        );
    };
